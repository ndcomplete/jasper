...
namespace JasperHttp.Routing
{
    public class RouteTree
    {
        ...
        public RouteTree(JasperHttpOptions options, GenerationRules rules)
        {
            foreach (var chain in options.Routes) chain.Route.Place(this);
        ...
        public void CompileAll(IContainer container)
        {
            ...
            foreach (var route in _options.Routes) route.AssemblyType(_assembly, connegRules, _rules, container);

            foreach (var methodRoutes in _methods) methodRoutes.AssemblySelector(_assembly, _options.Routes);
